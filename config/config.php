...
$management_token = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpYXQiOjE3MDQ2ODk3ODIsImV4cCI6MTcwNDc3NjE4MiwianRpIjoiand0X25vbmNlIiwidHlwZSI6Im1hbmFnZW1lbnQiLCJ2ZXJzaW9uIjoyLCJuYmYiOjE3MDQ2ODk3ODIsImFjY2Vzc19rZXkiOiI2NTYwOGY4MzY4MTExZjZmZTRiNTdmOWIifQ.ZES7Ks0wkS4KvwQ9XxopOhzH68azqWygghs68f0xa08';
...
function get_user_info($id)
{
  ...
  $course = $role === 'counselor' ? "" : "course_code,";

  $result = $schoolConn->query("SELECT DATE_FORMAT(birth_date, '%M %e, %Y') AS birth_date, {$role}_address,$course  {$role}_gender FROM $role WHERE {$role}_id = '$id'");
  ...
  return array(
    ...
    'address' => $data_school[0]["{$role}_address"],
    'course_code' => $role === 'counselor' ? "" : $data_school[0]['course_code'],
    'gender' => $data_school[0]["{$role}_gender"],
  );
}
...
function get_all_appointments($id, $role)
{
  global $mainConn;

  $arg = $role === 'counselor' ? 'counselor_id' : 'creator_id';
  $arg1 = $role === 'counselor' ? 'creator_id' : 'counselor_id';
  $result = $mainConn->query("SELECT * FROM appointment INNER JOIN user ON user.user_id = appointment.$arg1 WHERE $arg = '$id'");
...
function get_appointments($id, $limit)
{
  ...
  $args = $_SESSION['role'] === 'counselor' ? 'counselor_id ' : 'creator_id';

  $result = $mainConn->query("SELECT * FROM appointment INNER JOIN user ON user.user_id = appointment.counselor_id WHERE $args = '$id'");
...
function get_todays_appointments($id, $role)
{
  global $mainConn;
  ...
  $result = $mainConn->query("SELECT * FROM appointment INNER JOIN user ON user.user_id = appointment.$arg1 WHERE appointment_date = '$date' AND $arg = '$id'");
  if ($result->num_rows > 0) {
    return $result->fetch_all(MYSQLI_ASSOC);
  }
...
function get_tomorrow_appointments($id, $role)
{
  global $mainConn;

  $arg = $role === 'counselor' ? 'counselor_id' : 'creator_id';
  $arg1 = $role === 'counselor' ? 'creator_id' : 'counselor_id';
  $result = $mainConn->query("SELECT * FROM appointment INNER JOIN user ON user.user_id = appointment.$arg1 WHERE DATE(appointment_date) = DATE_ADD(CURDATE(), INTERVAL 1 DAY) AND $arg = '$id'");
...
function get_unfinished_appointments($id, $role)
{
  global $mainConn;

  $arg = $role === 'counselor' ? 'counselor_id' : 'creator_id';
  $arg1 = $role === 'counselor' ? 'creator_id' : 'counselor_id';
  $result = $mainConn->query("SELECT * FROM appointment INNER JOIN user ON user.user_id = appointment.$arg1 WHERE status = 'unfinished' AND $arg = '$id'");
...
function get_finished_appointments($id, $role)
{
  global $mainConn;

  $arg = $role === 'counselor' ? 'counselor_id' : 'creator_id';
  $arg1 = $role === 'counselor' ? 'creator_id' : 'counselor_id';
  $result = $mainConn->query("SELECT * FROM appointment INNER JOIN user ON user.user_id = appointment.$arg1 WHERE status = 'finished' AND $arg = '$id'");
