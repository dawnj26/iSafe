...
switch ($filter) {
    case 'all':
        ...
        break;
    case 'today':
        $appointments = get_todays_appointments($id, $role);
        //        print);
        //        print_r($appointments);
        display_appointments($appointments);
        break;
    case 'tomorrow':
        $appointments = get_tomorrow_appointments($id, $role);
        //        print);
        //        print_r($appointments);
        display_appointments($appointments);
        break;
    case 'unfinished':
        $appointments = get_unfinished_appointments($id, $role);
        //        print);
        //        print_r($appointments);
        display_appointments($appointments);
        break;
    case 'finished':
        $appointments = get_finished_appointments($id, $role);
        //        print);
        //        print_r($appointments);
        display_appointments($appointments);
        break;
...
function display_appointments($appointments)
{
    foreach ($appointments as $appointment) {
        ...
        $is_finished = $status == 'finished' ? 'disabled': '';
        $unfinished = "
        ...
        echo "</td>
					<td>
						<div class=''>
							<button type='button' onclick='deleteAppointment($id)' class='delete flex items-center gap-2 bg-error-600 hover:bg-error-700 px-4 py-2 rounded-lg text-white font-medium disabled:bg-gray-400' value='$id' $is_finished>
